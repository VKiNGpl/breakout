...
function Brick:hit()
    ...
    if self.isLocked and not self.keyActive then
        self.psystem:emit(4)
        gSounds['brick-hit-2']:stop()
        gSounds['brick-hit-2']:play()
    elseif self.isLocked and self.keyActive then
        self.psystem:emit(64)
        gSounds['locked-hit-1']:stop()
        gSounds['locked-hit-1']:play()
        self.inPlay = false
    else
...
function Brick:render()
    if self.inPlay then
        ...
        elseif self.isLocked and self.keyActive then
            love.graphics.draw(gTextures['main'],
                gFrames['bricks'][21], self.x, self.y)
        else
        love.graphics.draw(gTextures['main'], 
